        ...
        client.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            for (const info of messages) {
                try {
                    ...
                    // Marca como lida (não bloqueia se falhar)
                    try {
                    ...
                    if (ctx.body && !ctx.isCmd) {
                        ...
                        for (const [, intencao] of Object.entries(intencoes)) {
                            const encontrou = intencao.padroes.some((padrao) =>
                                padrao.test(texto)
                            )
                    ...
                    if (!comando) {
                        // Comando desconhecido — opcionalmente avisar
                        // await ctx.reply(`❌ Comando *${ctx.prefix}${ctx.comando}* não encontrado.\nDigite *${ctx.prefix}menu* para ver a lista.`)
                        continue
                    }
